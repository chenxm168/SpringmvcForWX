...
public class TibClient implements TibrvErrorCallback, ITibInterface,
	...
	private TibrvTransport getTransport(String envString, String module, int seq) {
		...
		try {
			...
			log.info(String.format("Creat transport!key:%s;transport:%s", key,transport));
			// 20210121 add
			if (tspMap.containsKey(key)) {
				try {
					tspMap.remove(key);
			...
			try {
				if (!tspMap.containsKey(key)) {
					try {
						...
						log.info(String.format("put transport into map;key:%s;transport:%s", key,transport.toString()));
